...
def parse_args():
    ...
    parser.add_argument('--test-feedback', type=str, default='argmax', choices=['argmax', 'sample'])
    parser.add_argument('--no-pos-emb', action='store_true', default=False)
    parser.add_argument('--no-gas', action='store_true', default=False)
    parser.add_argument('--hist-envdrop', action='store_true', default=False)
    parser.add_argument('--cand-use-ob-pos', action='store_true', default=False)
    parser.add_argument('--r4r-testall', action='store_true', default=False)
    parser.add_argument('--multiple-saving', action='store_true', default=False)
    parser.add_argument('--baseline_sr', type=float, default=45.0)
    parser.add_argument('--debug', action='store_true', default=False)
    parser.add_argument('--use-conn', dest='use_conn', action='store_true')
    parser.add_argument('--no-conn', dest='use_conn', action='store_false')
    parser.set_defaults(use_conn=True)
    parser.add_argument('--rl_teacher_weight', type=float, default=0.2)
    parser.add_argument('--rl_temperature', type=float, default=1.0)
    parser.add_argument('--no-hist-mapping', action='store_true', default=False)
    parser.add_argument('--rl_teacher_only', action='store_true', default=False)
    parser.add_argument('--no-temporal', action="store_true", default=False)
    parser.add_argument('--no-temporal-strategy', type=str, default='keep', choices=['replace', 'keep'])
    parser.add_argument('--no-path-restriction', action='store_true', default=False)
    parser.add_argument('--logit-reweighting', action='store_true', default=False)

    parser.add_argument('--global-positions', action='store_true', default=False)
    parser.add_argument('--gp_spacing', type=float, default=6)
    parser.add_argument('--gp_grid_size', type=int, default=5)
    parser.add_argument('--gp_loss_weight', type=float, default=0.2)

    parser.add_argument('--root_dir', type=str, default='../datasets')
    parser.add_argument(
        '--dataset', type=str, default='r2r', 
        choices=['r2r', 'r4r', 'r2r_back', 'r2r_last', 'rxr', 'reverie']
    )
