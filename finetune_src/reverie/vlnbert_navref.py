...
class NavRefCMT(BertPreTrainedModel):
    ...
    def forward(self, mode, txt_ids=None, txt_embeds=None, txt_masks=None,
                ...
                ob_img_feats=None, ob_ang_feats=None, ob_nav_types=None, ob_masks=None, ob_position_feat=None,
                obj_feats=None, obj_angles=None, obj_poses=None, obj_masks=None, obj_position_feat=None,
                global_pos_feat=None, graph_mask=None, history_mapper=None):
        ...
        if mode == 'history':
            ...
            if global_pos_feat is not None:
                ...
                vision_embeds = torch.cat([hist_embeds, ob_embeds, obj_embeds, global_pos_embeds], 1)
                extended_vision_masks = torch.cat([extended_hist_masks, extended_ob_masks, extended_obj_masks, global_pos_mask], -1)
            else:
            ...
            extended_vision_masks_graph = extended_vision_masks.transpose(-1, -2) * extended_vision_masks
            if graph_mask is not None:
                graph_max_size = graph_mask.size(-1)
                extended_vision_masks_graph[:, 0, 1:graph_max_size + 1, 1:graph_max_size + 1] *= graph_mask
            extended_vision_masks = (1.0 - extended_vision_masks) * -10000.0
            extended_vision_masks_graph = (1.0 - extended_vision_masks_graph) * -10000.0
            ...
            for layer_module in self.encoder.x_layers:
                txt_embeds, vision_embeds = layer_module(
                    ...
                    vision_embeds, extended_vision_masks, visn_self_attn_mask=extended_vision_masks_graph
                )

            hist_embeds = vision_embeds[:, :hist_max_len]
            ob_embeds = vision_embeds[:, hist_max_len: hist_max_len+ob_max_len]
            obj_embeds = vision_embeds[:, hist_max_len+ob_max_len:hist_max_len+ob_max_len+obj_max_len]
            if global_pos_feat is not None:
                pos_embeds = vision_embeds[:, hist_max_len+ob_max_len+obj_max_len:]
            ...
            if global_pos_feat is not None:
                return act_logits, obj_logits, txt_embeds, hist_embeds, ob_embeds, obj_embeds, pos_embeds
