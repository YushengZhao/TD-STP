...
def build_dataset(args, rank=0, is_test=False):
    ...
    train_instr_data = construct_instrs(
        args.anno_dir, args.dataset, [train_env_name],
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    best_val = {'val_unseen': {"spl": 0., "rgspl": 0., "state": "", "sr": 0.}}
    ...
    for idx in range(start_iter, start_iter + args.iters, args.log_every):
        ...
        if default_gpu:
            ...
            GP_loss = sum(listner.logs['GP_loss']) / max(len(listner.logs['GP_loss']), 1)
            ...
            write_to_record_file(
                "\ntotal_actions %d, max_length %d, entropy %.4f, IL_loss %.4f, REF_loss %.4f, RL_loss %.4f, GP_loss %.4f, policy_loss %.4f, critic_loss %.4f" % (
                    total, length, entropy, IL_loss, REF_loss, RL_loss, GP_loss, policy_loss, critic_loss),
                record_file
            )
        ...
        for env_name, env in val_envs.items():
            ...
            if default_gpu:
                ...
                if env_name in best_val:
                    if score_summary['spl'] + score_summary['sr'] >= best_val[env_name]['spl'] + best_val[env_name][
                        'sr']:
                    ...
                    if args.multiple_saving and score_summary['sr'] > args.baseline_sr:
                        listner.save(idx, os.path.join(args.ckpt_dir, "best_%s_%s" % (env_name, iter)))
